...
# library("gelnetMioPkg")
# To use if my package is not installed!
library(Rcpp)
...
bic_gelnet = function(X, ebic_par = 0, algorithm = 1, alpha_seq = NULL, lambda_seq = NULL, toler = 1e-5, maxit = 500){
  ...
  for(ii in alpha_seq){
    ...
    for(jj in lambda_seq){
      ...
      if(algorithm==1){
        O_ETH_pp_init = solve(diag(diag(S+jj*ii*diag(p))))
        # To ensure a symmetric matrix
        O_ETH_pp_init = (O_ETH_pp_init+t(O_ETH_pp_init))/2
        W_ETH_pp_init = S + jj*ii*diag(p) + 2*jj*(1-ii)*O_ETH_pp_init
        full_out = gelnet_mio_cpp(sampleCov=S, Winit=W_ETH_pp_init, Oinit=O_ETH_pp_init, alpha=ii, lambda=jj, toler=toler, maxiters=maxit, penalizediagonal=TRUE)
      }
      if(algorithm==2){
